...
- Never permanently store: customer passwords, API keys, authentication tokens, or secrets.
- Never permanently store: production credentials or infrastructure secrets.
- Never permanently store: internal security vulnerabilities until publicly disclosed or resolved.
- Never permanently store: employee HR discussions, performance reviews, or compensation information.
- Never permanently store: legal investigations or confidential legal documents.
- Never permanently store: acquisition, fundraising, or M&A discussions before they become public.
- Never permanently store: customer contract terms unless directly relevant to a product decision.
- Never permanently store: any information explicitly marked Restricted or Need-to-Know.
- Sensitive information required for in-session reasoning may be used transiently. Exclude from durable knowledge base unless PM explicitly instructs otherwise.
